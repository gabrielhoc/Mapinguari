...
rvertnet_download_for_cleanpoints <- function(genus_value,
                                             species_value,
                                             max_number_of_records_value,
                                             country_value = NULL,
                                             province_state_value =  NULL,
                                             only_with_lat_log_logical,
                                             out_dir_value = NULL) {
...
term_tibble_raw <- searchbyterm(specificepithet = species_name, genus = genus_name, 
                    cntry = country_name, stateprovince = province_state_value,
                    mappable = only_with_lat_log, limit = max_number)

## post process after rvernet

term_tibble_data <- term_tibble_raw$data
tibble_data_df_raw <- data.frame(genus = term_tibble_data$genus, species = term_tibble_data$specificepithet, Lon = term_tibble_data$decimallongitude, Lat = term_tibble_data$decimallatitude, stringsAsFactors = FALSE)
...
if(is.null(out_dir) == FALSE) {
                              ...
                              setwd(out_dir)
                              write.table(tibble_data_df_2, file = paste0(species_name_ref, "_total_", n_entries, "_unique_vernet.txt"), sep = "\t", row.names = FALSE)
...
cleanpoints_expanded <- function(spcoord,
                        ...
                        IUCN_range_raster_map_value = NULL,
...
if("species" %in% names(spcoord_table) == FALSE) {
            ...
            if("species" %in% correct_names == FALSE) {correct_names <- gsub("genus_species", "species", change_names)}
            if("species" %in% correct_names == FALSE) {correct_names <- gsub("Genus_species", "species", change_names)}
            if("species" %in% correct_names == FALSE) {correct_names <- gsub("Genus_Species", "species", change_names)}
            if("species" %in% correct_names == FALSE) {correct_names <- gsub("genus_species", "species", change_names)}
            if("species" %in% correct_names == FALSE) {stop("please check that species names as -species- column exist")}
...
if(class(species_split) =="try-error") {stop("the input species coordinates file or data frame is incorrect")}
...
if (plot_clean_data == TRUE) {
    if(!is.null(IUCN_range_raster_map)) {
                                ...
                                points(species_selected_coordinates$Lon, species_selected_coordinates$Lat, pch=1, cex=0.75)
