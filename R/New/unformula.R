...
for (i in 1:length(as.list(formula3)[[3]])) {
    print(as.list(formula4[[3]][i]))
}
...
unformula2 <- function(formula) {
    ...
    return(list(response = response_var, smooth = smooth_vars, random = random_vars, linear = linear_vars))
