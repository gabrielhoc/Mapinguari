...
*Fulanus beltranus* is a fictional species of lizard from central Africa. We would like to find out how this species is going to do in different climate change scenarios. First let's look at the points where it is currently known to occurr. To access the distribution data, we must load package Mapinguari.

```{r, message = FALSE, warning = FALSE, fig.height = 6, fig.width = 6}
library(Mapinguari)

Fulanus_bbox <- ggmap::make_bbox(lat = Lat, lon = Lon, data = FulanusDistribution, f = 0.2)

Fulanus_big <- ggmap::get_map(location = Fulanus_bbox, source = "google", maptype = "terrain")

ggmap::ggmap(Fulanus_big) +
  ggplot2::geom_point(data = FulanusDistribution, mapping = ggplot2::aes(x = Lon, y = Lat), size = 0.5, colour = 'red')
```
...
raster::plot(raster::stack(
  FulanusEcoRasters_future$`2050_rcp26`$tmax_2050_rcp26, 
  FulanusEcoRasters_future$`2050_rcp45`$tmax_2050_rcp45, 
  FulanusEcoRasters_future$`2050_rcp85`$tmax_2050_rcp85))
```
...
raster::plot(raster::stack(
  FulanusEcoRasters_derived$present$PET_present, 
  FulanusEcoRasters_derived$present$AET_present, 
  FulanusEcoRasters_derived$present$CWD_present))
```

You might have noticed the argument `phenology`, which in our examples is set to 'year'. That means the `non-fixed` variables, which are usually in monthly resolution, will be averaged accross the year. If you want to preserve them in monthly resolution, you can set it to `month`. There is another setting, which is `season`, that I will describe in more detail when talking about the `Phenology` function.
...
This closure can be combined with the output from `Ecology` to create a raster of physiology, ehich varies according to local environmental conditions. This is the subject of the next function.
...
raster::plot(Perf_rasters_month[[1]])
```
...
PhenFUN1 <- function(x) 1/(1 + exp((300 - x)/2))

Seasons_by_rain <-
Phenology(raster_list = FulanusEcoRasters_month,
  PhenFUN = PhenFUN1 ,
  Phen_args = list(x = 'prec'))

# Let's say we are only interested on the periods when performance is positive.

PhenFUN2 <- function(x) ifelse(x > 0, 1, 0)

Seasons_by_performance <-
Phenology(raster_list = Perf_rasters_month,
  PhenFUN = PhenFUN2 ,
  Phen_args = list(x = 'Perf'))

raster::plot(Seasons_by_rain[[1]])

raster::plot(Seasons_by_performance[[1]])
```
...
**This is a theoretical description of the package from my thesis proposal, I need to update it**
