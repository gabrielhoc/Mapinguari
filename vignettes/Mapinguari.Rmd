...
Fulanus_future_year_summaries_list <-   
...
plot(Fulanus_present_year_summaries$prec_year_sum)
plot(Fulanus_present_two_seasons_summaries$prec_breeding_sum)
plot(Fulanus_present_two_seasons_summaries$prec_growing_sum)
```
...
plot(Perf_rasters_tmax$perf_year_mean)
plot(Perf_rasters_tmin$perf_year_mean)
```

The function operates in RasterStacks, if you want to apply it to a list, you can use `lapply`:

```{r, eval, message = FALSE, warning = FALSE, fig.height = 6, fig.width = 6}
FulanusEcoRasters_future_temps <- lapply(Fulanus_future_year_summaries_list, function(x) x[[2:4]]/10)
...
FulanusEcoRasters_season_rain <-
    summarize_rasters(FulanusEcoRasters_present$present,
      seasons = list(year = c(1:12)),
...
plot(FulanusEcoRasters_season_rain$tmax_year_weighted.mean)
```
...
FulanusEcoRasters_season_prec <-
    summarize_rasters(FulanusEcoRasters_present$present[[1:12]],
      seasons = list(year = c(1:12)),
      summaryFUN = "weighted.mean", 
      summary_args = list(weights = season_prec_rasters))
...
FulanusMicroclimate$HA <- ifelse(FulanusMicroclimate$temp > 21 & FulanusMicroclimate$temp < 33, 1, 0)
...
HA_curve <- 
fit_curves(HA ~ SSlogis(avg_temp, Asym, xmid, scal), 
  data = microclimate_by_day, 
  fitFUN = nls,
...
HA_EVI_curve <- 
fit_curves(HA ~ SSlogis(avg_temp + EVI, Asym, xmid, scal), 
  data = microclimate_by_day, 
  fitFUN = nls,
...
Fulanus_present_year_averages <-
   summarize_rasters(FulanusEcoRasters_present[[1]],
    seasons = list(year = c(1:12)),
    summaryFUN = list(tmax = "mean", prec = "mean"))

HA_raster_present <-
  transform_rasters(raster_stack = Fulanus_present_year_averages,
    transformFUN = list(HA = HA_EVI_curve$model_1$predict),
    transformFUN_args = list(temp = 'tmax', EVI = 'prec'), # 'prec' is here as a placeholder, get real EVI
    alert = 1)
