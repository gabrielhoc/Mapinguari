...
# Let's scale the tmin and tmax layers
FulanusEcoRasters_present_temps <- FulanusEcoRasters_present$present[[13:36]]/10
...
# Let's try first with a binomial glm that we are going to fit for the dataset `FulanusBreeding` that contains a binary variable indicating if Fulanus was breeding at each month, as well as corresponding climate data
...
Seasons_by_rain <-
    summarize_rasters(FulanusEcoRasters_present$present,
      ...
      seasons_args = list(prec = 'prec'),
      summaryFUN = "mean")

# a new `season_length` layer will be created, with the duration of the season.

raster::plot(Seasons_by_rain$phen_breeding_mean)
raster::plot(Seasons_by_rain$prec_breeding_mean)
```
...
Steps:
-Use `transform_rasters` to calculate EVI (Find EVI equation, if can't find it, use NDVI instead);
