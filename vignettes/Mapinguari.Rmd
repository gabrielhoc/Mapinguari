...
The predictor function can be applied to rasters of the variables on the right hand side of the model formula to spatialize the variable on the right hand side. On the next section we are going to give an example.
...
Now, let's say the most crucial period for *Fulanus* population dynamics is their breeding period and they will breed only under certain climatic conditions. This might cause their breeding period to vary spatially and temporally. In order to account for that, we can fit a logistic model of breeding status vs climate, using data from the table `FulanusBreeding`, which contain data on breeding status and climate at different localities. Then we can use this model to create binary rasters with the locations where Fulanus is breeding at each month, using function `transform_rasters`. After that, we can summary the climatic rasters only during the breeding season at different places, by using the binary rasters as weights to average other variables. 
...
HAlogistic <- nls(HA ~ SSlogis(temp_air_avg, Asym, xmid, scal), data = microclimate_by_day)
...
HAlogis <- HA_curve$logistic$predict
...
HA_raster_present <-
  transform_rasters(raster_stack = Fulanus_present_year_averages,
    FUN_qlist = qlist(HA = HAlogis(tmax/10)))
...
# Let's try also including CWD, which is a proxy for vegetation
...
# let's include a new column in the table with CWD
microclimate_by_day$PET <- raster::extract(mean(PET_stack), data.frame(Lon = microclimate_by_day$Lon, Lat = microclimate_by_day$Lat))

# let's include a new column in the table with CWD
microclimate_by_day$AET <- raster::extract(mean(AET_stack), data.frame(Lon = microclimate_by_day$Lon, Lat = microclimate_by_day$Lat))

# Let's try fitting a curve with CWD

HAlogisticCWD <- nls(HA ~ SSlogis(temp_air_avg + CWD, Asym, xmid, scal), data = microclimate_by_day)
HAGompertzCWD <- nls(HA ~ SSposnegRichards(temp_air_avg + CWD, Asym, K, Infl, modno = 32, pn.options = "G_options"), data = microclimate_by_day)
HAVonBertalannfyCWD <- nls(HA ~ SSposnegRichards(temp_air_avg + CWD, Asym, K, Infl, modno = 32, pn.options = "VB_options"), data = microclimate_by_day)
HARichardsCWD <- nls(HA ~ SSposnegRichards(temp_air_avg + CWD, Asym, K, Infl, M, modno = 12, pn.options = "R_options"), data = microclimate_by_day)

list(HAlogisticCWD, HAGompertzCWD, HAVonBertalannfyCWD, HARichardsCWD)

HA_CWD_curve <- 
fit_curves(list(HAlogisticCWD, 
                HAGompertzCWD, 
                HAVonBertalannfyCWD, 
                HARichardsCWD), 
...
Fulanus_present_year_averages <-
   transform_rasters(FulanusEcoRasters_present_CWD$present,
    FUN_qlist = qlist(tmax_average = mean(tmax),
                      CWD_average = mean(CWD)))
...
Fulanus_present_year_averages_scaled$CWD <- mean(CWD_stack)

microtemp_raster_present <-
  transform_rasters(raster_stack = Fulanus_present_year_averages_scaled,
    FUN_qlist = qlist(microtemp = predict_microtemp(temp_air_avg = tmax, CWD = CWD))) 

names(microtemp_raster_present) <- "microtemp"
...
Some points might be too close to each other and be redundant, while other might be mistakenly placed in regions where the species is known not to occurr. Fulanus only occurrs between the altitudes of 500 and 1000 meters. Let's filter out points that are within 2 kilometers from each other and outside the altitude range where the species is know to occurr. 

```{r, message = FALSE, warning = FALSE, fig.height = 6, fig.width = 6}
# A reference layer has to be inputed 
...
SDM_Fulanus1 <- 
fit_curves(list(
  glm(presence ~ HA + HR + perf + breed + PET + CWD, data = pres_abs_table, family = "binomial"),
  glm(presence ~ HA + HR + perf + breed + PET, data = pres_abs_table, family = "binomial"),
  glm(presence ~ HA + HR + perf + breed + CWD, data = pres_abs_table, family = "binomial"),
  glm(presence ~ HA + HR + perf + PET + CWD, data = pres_abs_table, family = "binomial"),
  glm(presence ~ HA + HR + breed + PET + CWD, data = pres_abs_table, family = "binomial"),
  glm(presence ~ HA + perf + breed + PET + CWD, data = pres_abs_table, family = "binomial"),
  glm(presence ~ HR + perf + breed + PET + CWD, data = pres_abs_table, family = "binomial")))
  
SDM_Fulanus2 <- 
fit_curves(list(
  glm(presence ~ HA + HR + perf + breed + PET, data = pres_abs_table, family = "binomial"),
  glm(presence ~ HA + HR + perf + breed, data = pres_abs_table, family = "binomial"),
  glm(presence ~ HA + HR + perf + PET, data = pres_abs_table, family = "binomial"),
  glm(presence ~ HA + HR + breed + PET, data = pres_abs_table, family = "binomial"),
  glm(presence ~ HA + perf + breed + PET, data = pres_abs_table, family = "binomial"),
  glm(presence ~ HR + perf + breed + PET, data = pres_abs_table, family = "binomial")))

SDM_Fulanus3 <- 
fit_curves(list(
  glm(presence ~ HA + HR + perf + PET, data = pres_abs_table, family = "binomial"),
  glm(presence ~ HA + HR + perf, data = pres_abs_table, family = "binomial"),
  glm(presence ~ HA + HR + PET, data = pres_abs_table, family = "binomial"),
  glm(presence ~ HA + perf + PET, data = pres_abs_table, family = "binomial"),
  glm(presence ~ HR + perf + PET, data = pres_abs_table, family = "binomial")))

SDM_Fulanus4 <- 
fit_curves(list(
  glm(presence ~ HA + HR + perf, data = pres_abs_table, family = "binomial"),
  glm(presence ~ HA + HR, data = pres_abs_table, family = "binomial"),
  glm(presence ~ HA + perf, data = pres_abs_table, family = "binomial"),
  glm(presence ~ HR + perf, data = pres_abs_table, family = "binomial")))

SDM_Fulanus5 <- 
fit_curves(list(
  glm(presence ~ HA + HR, data = pres_abs_table, family = "binomial"),
  glm(presence ~ HA, data = pres_abs_table, family = "binomial"),
  glm(presence ~ HR, data = pres_abs_table, family = "binomial")))
  
SDM_predict <- SDM_Fulanus5$model_1$predict

SDM_projection <-
transform_rasters(total_stack, FUN_qlist = qlist(SDM_predict(HA, HR)))

raster::plot(SDM_projection)

# GBM e RF

SDM_FulanusGBMRF <- 
fit_curves(list(
  gbm(presence ~ HA + HR + perf + breed + PET + CWD, data = pres_abs_table),
  randomForest(presence ~ HA + HR + perf + breed + PET, data = pres_abs_table)))
  
GBM_predict <- SDM_FulanusGBMRF$model_1$predict
RF_predict<- SDM_FulanusGBMRF$model_2$predict

GBM_projection <-
transform_rasters(total_stack, FUN_qlist = qlist(GBM_predict(HA, HR, perf, breed, PET, CWD)))

raster::plot(GBM_projection)

RF_projection <-
transform_rasters(total_stack, FUN_qlist = qlist(RF_predict(HA, HR, perf, breed, PET, CWD)))

raster::plot(RF_projection)
